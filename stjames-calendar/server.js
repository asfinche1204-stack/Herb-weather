...
function htmlToLines(html) {
  ...
  h = h.replace(/<br\s*\/?>/gi,'\n').replace(/<\/(p|div|li|tr|h[1-6]|td|th|section|article|ul|table)>/gi,'\n').replace(/<[^>]*>/g,' ')
  ...
  // the page title appears in the menu first and as the real heading later: start from the LAST heading match
  let start = -1; all.forEach((l,i) => { if (/daily course conditions/i.test(l) && l.length < 60) start = i; });
  if (start < 0) start = 0;
  let end = all.findIndex((l,i) => i > start && /privacy policy|all rights reserved|©|powered by|back to top|site map|member login|log out/i.test(l));
  if (end < 0) end = all.length;
  const lines = all.slice(start, end).filter((l,i,a) => l !== a[i-1]);
  return { lines, all };
}
async function refreshConditions() {
  try {
    if (!jar.size) await login();
    let r = await get(`${BASE}/member-home/daily-course-conditions`);
    if (/login\.asp/i.test(r.url)) { await login(); r = await get(`${BASE}/member-home/daily-course-conditions`); }
...
async function login() {
  jar.clear(); loginDiag = { at: new Date().toISOString() };
  ...
  loginDiag.formFound = !!form; loginDiag.action = action; loginDiag.userField = userField; loginDiag.passField = passField; loginDiag.otherFields = [...fields.keys()];
  loginDiag.inputs = (form.match(/<input[^>]*>/gi) || []).map(i => i.replace(/value=["'][^"']*["']/i, 'value=…').slice(0, 160));
  ...
  storeCookies(r);
  loginDiag.postStatus = r.status; loginDiag.postLocation = r.headers.get('location'); loginDiag.cookies = [...jar.keys()];
  // verify against a members-only page: it must NOT show the login form
  const test = await get(`${BASE}/member-home`);
  const looksLoggedIn = /log\s*out|logout|sign\s*out/i.test(test.text) && !/type=["']?password/i.test(test.text);
  loginDiag.memberHomeUrl = test.url; loginDiag.loggedIn = looksLoggedIn;
  if (!looksLoggedIn) throw new Error('Login did not stick (check STJ_USER / STJ_PASS; see /login-check)');
}
