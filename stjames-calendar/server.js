...
function htmlToLines(html) {
  ...
  // keep only the main content when the page marks it
  const main = h.match(/<main[\s\S]*?<\/main>/i) || h.match(/id=["']?(?:content|mainContent|main-content)["']?[\s\S]*?<\/(?:div|section)>/i);
  if (main) h = main[0];
  h = h.replace(/<br\s*\/?>/gi,'\n').replace(/<\/(p|div|li|tr|h[1-6]|td|th|section)>/gi,'\n').replace(/<[^>]+>/g,' ')
       .replace(/&nbsp;/g,' ').replace(/&amp;/g,'&').replace(/&#39;|&rsquo;/g,"'").replace(/&quot;|&ldquo;|&rdquo;/g,'"').replace(/&[a-z]+;/g,' ');
  const lines = h.split('\n').map(l => l.replace(/\s+/g,' ').trim()).filter(Boolean);
  // trim boilerplate: start at the heading, stop at common footer phrases
  let start = lines.findIndex(l => /course conditions/i.test(l)); if (start < 0) start = 0;
  let end = lines.findIndex((l,i) => i > start && /privacy policy|all rights reserved|©|powered by/i.test(l)); if (end < 0) end = lines.length;
  return lines.slice(start, end).filter((l,i,a) => l !== a[i-1]);
}
async function refreshConditions() {
  try {
    ...
    const lines = htmlToLines(r.text);
...
app.get('/conditions.json', (req,res) => res.json(conditions));
