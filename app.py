...
def parse_sti(f, lat0, lon0):
    cells = {}
    # STI pages: "STORM ID  AZ/RAN  MOVEMENT  FCST 15  FCST 30  FCST 45  FCST 60" and "STORM ID ... MAX DBZ ... "
    for ln in tab_text(f):
        ...
        if m.group(5):  # moving from direction / speed in knots
            c['dir'] = int(m.group(5)); c['spd_kt'] = int(m.group(6))
        rest = m.group(7) or ''
        fc = re.findall(AZRAN, rest)[:4]
        for i, (a, r_) in enumerate(fc):
            fla, flo = offset(lat0, lon0, int(a), int(r_))
            c['fcst'].append({'min': (i+1)*15, 'lat': fla, 'lon': flo})
